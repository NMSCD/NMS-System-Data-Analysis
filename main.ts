...
function createRegionTxt(regionName: string, regionDataArray: SystemData[]): void {
	const headers = ['Other Systems', 'Missing Prefix'].map(heading => `==${heading}==`);
	const other: string[] = [];
	...
	for (const system of regionDataArray) {
		if (system['Correctly Prefixed']) {
			...
			if (system.Name) missing.push(`${system.Name} (Expected Prefix: ${expected.substring(1, expected.length - 1)})`);
		}
	}
